...
This chapter describes the core stages of the proposed animal re-identification pipeline, used in both the classification mode and counting mode: preprocessing, feature extraction (global and local), aggregation of local descriptors into Fisher vectors (FV) and geometric verification used as an additional consistency check and for scoring.


\section{Preprocessing}
Wildlife images are rarely captured under controlled conditions, illumination can vary significantly, there can be background clutter and/or occlusion. Preprocessing is used to reduce the variation and potential background bias.
...
Even if the images in a dataset are cropped, background can still dominate local or global feature extraction which can lead to background bias. To eliminate this effect, a segmentation-based pipeline is used to isolate the animal from the background. The pipeline first localizes the animal using a text prompted detector (GroundingDINO)~\cite{groundingdino} and then produces a pixel level mask using Segment Anything Model 2 (SAM2)~\cite{ravi2024sam2}. The mask is applied with soft edges to avoid introducing sharp boundaries that could create non-existent keypoints.
...
Segmentation was not applied to all datasets in this thesis since some already came segmented or, in case of CZoo, were already cropped so tightly that it was deemed unnecessary. 
...
In this thesis I used modern learning-based local feature extractors: SuperPoint\cite{DeTone2018SuperPoint}, DISK\cite{tyszkiewicz2020disklearninglocalfeatures} and ALIKED\cite{zhao2023alikedlighterkeypointdescriptor}. While each of those works well on its own, experiments have proven that they complement each other so they have been used as an ensemble. These methods were chosen because they work well out-of-the-box, requiring no additional training or fine tuning and also learning-based detectors generally perform better than traditional ones like SIFT.\cite{Lowe2004SIFT} 
...
SuperPoint\cite{DeTone2018SuperPoint} uses a single convolutional network (CNN) with two heads: one head produces a per-pixel interest-point score map and the other produces a per-pixel descriptor map. It is trained without manual keypoint labels by warping images with random homographies and enforcing that detections and descriptors remain consistent under those warps.

\subsubsection{DISK}
DISK (DIScrete Keypoints)\cite{tyszkiewicz2020disklearninglocalfeatures} is an end-to-end trainable framework that jointly learns keypoint detection and description. Its training objective directly optimizes matching quality across image pairs, making it well suited for retrieval and re-identification settings where reliable correspondence is required. This was experimentally confirmed when testing different matchers for use with geometric verification. 

\subsubsection{ALIKED}
ALIKED\cite{zhao2023alikedlighterkeypointdescriptor} is a lightweight learned detector-descriptor model designed for efficient local feature extraction. It uses a deformable transformation strategy to better adapt to geometric variation while keeping inference computationally practical. In this thesis ALIKED provided a strong speed-accuracy tradeoff while complementing SuperPoint and DISK in the ensemble setting. 
...
This gradient is formally known as the score function~\cite{springerSpeciesAgnosticPatterned}. It measures the sensitivity of the log-likelihood to changes in the model parameters, essentially indicating how to adjust the GMM to better fit the observed features. 
...
The matrix $L_\lambda$ is derived from the Fisher Information Matrix and is used to "whiten" or to normalize the gradients~\cite{5540009}, making them more discriminative and suitable for use in classification later on.
...
FV have also been chosen because they have been shown to perform better than bag-of-words approaches.~\cite{5540009}
...
Geometric verification is used to make sure that matched keypoints between images truly correspond to the same physical locations on an animal reducing false positives that arise from visual similarities alone. This step is crucial in refining the initial matches obtained through descriptor-based feature matching by enforcing spatial consistency.~\cite{springerSpeciesAgnosticPatterned}
...
Initial correspondences are established through descriptor matching using LightGlue~\cite{lindenberger2023lightgluelocalfeaturematching}, an attention-based method. Initially methods like Lowe's ratio test were considered but matching with LightGlue proved significantly faster and more accurate, effectively eliminating the need for traditional filtering methods.
