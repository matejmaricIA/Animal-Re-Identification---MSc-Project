...
> **T06 deliverables (🔵 in review):** `reid_demo/catalogue.py` + `tests/test_catalogue.py` (read-only static HTML catalogue). `build_catalogue(...) -> CatalogueResult` + `python -m reid_demo.catalogue` CLI (`--selftest`). Emits `index.html`/`individuals/`/`unassigned.html`/`thumbs/`/`assets/style.css`/`catalogue_summary.json` (+ optional `montages/*.png`), all relative paths (portable after copytree). Honors D5 (`is_candidate_new`→unassigned), D7c by_flank sum-invariant over `cluster_id>=0` rows, mixed-flank flag, missing-crop placeholders. SELECT-only (store byte-identical before/after build). No existing file modified (additive `reid_demo/__init__.py` re-exports done centrally).

> **T07 deliverables (🔵 in review):** `reid_demo/eval.py` + `tests/test_eval.py` (23 passed). `ClusteringReport` + `load_eval_frame`/`build_label_arrays`/`standard_metrics`/`plain_language_metrics`/`evaluate_clustering`/`save_report` + `python -m reid_demo.eval` CLI. Plain-language (individuals found/true, `pct_photos_correctly_grouped` 0–100, merge/split errors, candidate-new P/R with `None` semantics) + standard ML (homogeneity/completeness/V-measure/ARI/AMI + BCubed pairwise via the C(n,2) group-size formula, matched to brute-force). D4 `{left,right,other}` flank-aware toggle. Writes the single `evaluations/clustering/<dataset>_<tag>.json` (D6 headline trio). Read-only, store-API only.

> **T08 deliverables (🔵 in review):** `reid_demo/review.py` + `tests/test_review.py` (35 passed). Must-ship headless core: `build_review_queue`/`apply_decisions`/`review_status_summary`/`build_pair_image` + decisions-JSON round-trip + `python -m reid_demo.review` CLI (`--build-queue`/`--apply`/`--status`); `serve_review_ui` present (stdlib `http.server`, optional). Keys singletons on `is_candidate_new==1` (D5), respects flanks (never pairs left↔right), merge=smaller-id-wins, split/new=`max+1`, idempotent re-apply, optional D8 `pair_scores` ordering hint. Writes only via `update_review`; no web framework; no T11/T12 import (hard deps stay T01/T02/T05).

> **T12 deliverables (🔵 in review):** `reid_demo/fusion.py` + `tests/test_fusion.py` (GV stubbed; LightGlue optional). Pure cores `build_fused_affinity`/`affinity_provider`/`select_borderline_pairs`/`gv_rerank`/`refine_affinity_with_gv` + driver `run_fusion`/`load_affinity` + `python -m reid_demo.fusion` CLI. Calibrated global+Fisher Tier-2 mean affinity (D8) consumable by T05's pluggable `affinity=` arg (sorted-id `(N,N)` `.npz`); budget-capped borderline GV reranking (never N²) → `…_pairs.json` sorted by `geom_score`; graceful no-op without LightGlue (`gv_ran=False`). Flank gating (left↔right=0.0). Writes NO `detections` columns; `reid_demo/cluster.py` (T05) unmodified and imports no fusion (no cycle).
...
> **T10 deliverables (🔵 in review):** `reid_demo/run_demo.py` + `tests/test_run_demo.py` (27 passed; stages stubbed, no GPU/model runs). The conductor: `build_config`/`plan_stages`/`run_demo`/`assemble_bundle`/`main` + `DemoConfig`/`StageSpec`/`StageResult`/`BundleResult` + `python -m reid_demo.run_demo` CLI (`--config/--datasets/--primary-dataset/--db/--out-dir/--run-name/--stages/--signals/--smoke/--max-identities/--force/--dry-run/--continue-on-error`). Single-source `STAGE_REGISTRY` wires the real T02–T12 CLIs/functions (verified flag names); `DATASET_TRACKS`/`B_TRACK_SPECIES` drive B-track (`ingest→species(set_known)→embed→[fisher→fusion]→cluster→{catalogue,eval}`) vs A-track (`report`). D8 `--signals {global|global+fisher|full-funnel}`: `global` default (backbone, no T11/T12); funnel sets run a baseline global cluster+eval first, then T11→T12 fused (in-process `affinity=`) re-cluster+eval, and the bundle reports the **lift**. Assembles a static `demo_bundle/<run>/` (`index.html`+`SUMMARY.md`+`manifest.json`+`catalogue/`+`accuracy/`+`medvednica_report/`+`logs/`); headline trio read verbatim from T07's single `<dataset>_<tag>.json` (D6, 0–100 scale), `candidate_new`/`store_counts` from T01 `count_by`. Skip-if-exists (outputs or store-column populated) + `--force`; `--dry-run` prints the ordered plan and writes nothing; `--continue-on-error`→`status:"partial"`+exit 0, critical failure→`"failed"`+non-zero (bundle still written). Adds only T10 files; no existing module modified.
